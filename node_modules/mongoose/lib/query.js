 ...
 * #### Example:
 *
 *     const q = Magazine.findOne({ title: 'Casino Royale' });
 *     await q.exec();
 ...
 * #### Example:
 *
 *     let magazine = await Magazine.findOne().populate('authors');
 *     magazine.title; // 'Node.js in Action'
 *     magazine.authors[0].name; // 'TJ Holowaychuk'
 *     magazine.authors[1].name; // 'Nathan Rajlich'
 *
 *     let magazines = await Magazine.find().populate({
 *       path: 'authors',
 *       // `match` and `sort` apply to the Author model,
 *       // not the Magazine model. These options do not affect
 *       // which documents are in `magazines`, just the order and
 *       // contents of each magazine document's `authors`.
 *       match: { name: new RegExp('.*h.*', 'i') },
 *       sort: { name: -1 }
 *     });
 *     magazines[0].title; // 'Node.js in Action'
 *     // Each magazine's `authors` are sorted by name, descending.
 *     magazines[0].authors[0].name; // 'TJ Holowaychuk'
 *     magazines[0].authors[1].name; // 'Marc Harter'
 *
 *     magazines[1].title; // 'Professional AngularJS'
 ...
 * #### Example:
 *
 *      magazineSchema.pre('findOne', function() {
 *        let keys = this.getPopulatedPaths(); // ['author']
 *      });
 *      ...
 *      Magazine.findOne({}).populate('author');
 *
 * #### Example:
 *
 *      // Deep populate
